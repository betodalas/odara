...
df = df.groupby('data').agg(','.join).reset_index()
...
#        compara_soma['D1'] = "=SUM(C1:C100)"
#    return compara_soma
#soma2 = get_soma(soma2)
#print (soma2)
...
def get_soma2(soma2):
    ...
    for row in range(2, soma2.max_row + 1):
        _date = soma2["A" + str(row)]
        somas2.append((_date.value))

    return somas2

somas2 = get_soma2(soma2=userSheet)
...
#    DtcellObject1 = bankSheet["A" + str(row)]
#    ChcellObject = userSheet["B" + str(row)]        #get cell Object for every record found on column B of excel sheet
    AmcellObject3 = bankSheet["C" + str(row)]        #same as above for every record on column C of the excel file

    soma2 = (AmcellObject3.value)
...
time.sleep(1)
workBook.save("/var/www/odara/html/conciliacao.xlsx")             # create new file with all the matched instance highlighted automatically
